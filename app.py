...
st.markdown("## My XGBoost Example")
...
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown("### Select Input Variable")
cols = st.multiselect("독립 변수를 선택하세요", df.columns[1:])
...
# rscv = RandomizedSearchCV(regressor, params)
# rscv.fit(Xt, yt)
xgb.fit(Xt, yt)
...
# yt_pred = rscv.predict(Xt)
# yts_pred = rscv.predict(Xts)
